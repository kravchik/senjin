...
public class ShaderTranslator {
    ...
    private String translate(String srcPath) {
        ...
        int version = shaderType.equals("gs") ? 400 : 120;
        ...
        if (inputClass != StandardVertexData.class) for (Field fn : TypeUtils.getFieldsForData(inputClass)) {
            ...
            if ("vs".equals(shaderType)) {
                ...
                result += "varying " + type + " " + name + ";\n";
                varyingFS.add(name);
            } else throw BadException.shouldNeverReachHere();
        }
        if ("fs".equals(shaderType) && outputClass != StandardFSOutput.class) {
            throw new RuntimeException("Cannot use custom shader output in glsl 1.20");
        }
        if (outputClass != StandardFragmentData.class && outputClass != StandardFSOutput.class) {
            for (Field fn : outputClass.getDeclaredFields()) {
                ...
                String type = translateType(fn.getType().getName());
                result += "varying " + type + " " + withOutputSuffix(fn.getName()) + ";\n";
            }
        }
