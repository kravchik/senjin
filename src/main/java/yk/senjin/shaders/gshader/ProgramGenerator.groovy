...
//        new VertexShader().m()
        ...
        for (MethodNode m in mainClass.getMethods("main")) {
            if ((m.modifiers & Modifier.STATIC) != 0) continue
        ...
        for (o in mainClass.getFields()) {
            ...
            if ((fn.modifiers & Modifier.FINAL) != 0) {
                result += "const " + fn.name + " = " + translateExpression(((FieldNode) fn).initialExpression) + ";\n"
...
//                if (type.equals("vec4")) uniforms.add(new Uniform4fG(fn.name, (Vec4f) got))
        ...
        for (m in mainClass.getMethods("main")) {
            if ((m.modifiers & Modifier.STATIC) != 0) continue
...
//        result += "varying: " + varying + "\n"
    ...
    private String translateExpression(Object o) {
        ...
        if (o instanceof ConstantExpression) return ((ConstantExpression)o).text
